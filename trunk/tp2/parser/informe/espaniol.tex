...
\texttt{./opennlp ChunkerTrainerME -encoding UTF-8 -lang es -data X.train -model es-chunker.bin}

Mientras que para evaluar los archivos de test:

\texttt{./opennlp ChunkerEvaluator -encoding UTF-8 -data X.test -model es-chunker.bin}
...
De la construcción algorítmica se pueden observar muy buenos resultados respecto al armado de frases nominales así también como de varias frases verbales. De todas maneras nos llamó la atención el tag ``O'' sobre algunos verbos del infinitivo como ``cumplir vmn0000 O'', ``comenzar vmn0000 O, a sps00 O , extraer vmn0000 O'' o ``haber van0000 O, perdido vmp00sm O''. 

De todas maneras por ser una construcción algorítmica y no un tagueo realizado por un conjunto de linguistas, el cual también puede poseer error pero se debería asemejar a un análisis más preciso, es altamente probable que varias sutilezas del lenguaje hayan sido ignoradas y de esta forma se fueron introduciendo algunos errores que no nos permitieron superar el 0.90 de precisión.
...
Uno de los textos seleccionados fue la descripción de la carrera (http://www.dc.uba.ar/aca/carr/grado/licenciatura/desc) porque nos pareció un texto sin muchas estructuras complejas en cuanto a la tokenización y sin caracteres extraños, pero con varias frases nominales que nos pareció interesante ver como eran resueltas. 
